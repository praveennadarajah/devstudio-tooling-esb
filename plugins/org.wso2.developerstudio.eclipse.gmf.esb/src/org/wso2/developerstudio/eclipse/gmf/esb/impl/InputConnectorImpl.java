...
public abstract class InputConnectorImpl extends EsbConnectorImpl implements InputConnector {
    ...
    public boolean shouldConnect(OutputConnector sourceEnd) {
        ...
        /*
         * Avoid connecting an endpoint to an element other than Send, Call, Clone mediator and LB, Failover,
         * RecipientList endpoint's output connector.
         */
        if (!(sourceEnd instanceof SendMediatorEndpointOutputConnector)
                && !(sourceEnd instanceof CallMediatorEndpointOutputConnector)
                && !(sourceEnd instanceof CloneMediatorTargetOutputConnector)
                && !(sourceEnd instanceof LoadBalanceEndPointOutputConnector)
                && !(sourceEnd instanceof FailoverEndPointOutputConnector)
                && !(sourceEnd instanceof RecipientListEndPointOutputConnector)
        ...
        if (this.eContainer.equals(container)) {
             ...
             * }
             */
            if ((getIncomingLinks().size() != 0)
                    && (getIncomingLinks().get(0).getSource().eContainer() instanceof EndPoint)) {
         ...
         */
        if ((sourceEnd instanceof ProxyOutputConnector || sourceEnd instanceof APIResourceOutputConnector)
                && this.eContainer.eContainer().eContainer() instanceof ProxyServiceFaultContainer) {
            return false;
        }
