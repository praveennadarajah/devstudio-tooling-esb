...
public class ValidateSchemaPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public ValidateSchemaPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject validateSchema,
            String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.ValidateSchema.Properties.validateSchemaKeyType)) {
                basePart.initValidateSchemaKeyType(
                        EEFUtils.choiceOfValues(validateSchema,
                                EsbPackage.eINSTANCE.getValidateSchema_ValidateSchemaKeyType()),
                        validateSchema.getValidateSchemaKeyType());
            }
            if (isAccessible(EsbViewsRepository.ValidateSchema.Properties.schemaKey)) {
                // init part
                schemaKeySettings = new EObjectFlatComboSettings(validateSchema,
                ...
                basePart.addFilterToSchemaKey(new ViewerFilter() {
                    ...
                    public boolean select(Viewer viewer, Object parentElement, Object element) {
                        return (element instanceof String && element.equals("")) //$NON-NLS-1$
                                || (element instanceof RegistryKeyProperty);
                    }
        ...
        if (EsbViewsRepository.ValidateSchema.Properties.schemaKey == event.getAffectedEditor()) {
            if (event.getKind() == PropertiesEditionEvent.SET) {
                ...
                EObjectPropertiesEditionContext context = new EObjectPropertiesEditionContext(editingContext, this,
                        eObject, editingContext.getAdapterFactory());
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getValidateSchema_ValidateSchemaKeyType().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject)
                    && isAccessible(EsbViewsRepository.ValidateSchema.Properties.validateSchemaKeyType))
                basePart.setValidateSchemaKeyType((KeyType) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getValidateSchema_SchemaKey().equals(msg.getFeature()) && basePart != null
                    && isAccessible(EsbViewsRepository.ValidateSchema.Properties.schemaKey))
                ...
                if (EsbViewsRepository.ValidateSchema.Properties.validateSchemaKeyType == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getValidateSchema_ValidateSchemaKeyType().getEAttributeType(),
                                (String) newValue);
                    }
