...
public class LogMediatorPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public LogMediatorPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject logMediator,
            String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.LogMediator.Properties.logCategory)) {
                basePart.initLogCategory(
                        EEFUtils.choiceOfValues(logMediator, EsbPackage.eINSTANCE.getLogMediator_LogCategory()),
                        logMediator.getLogCategory());
            }
            if (isAccessible(EsbViewsRepository.LogMediator.Properties.logLevel)) {
                basePart.initLogLevel(
                        EEFUtils.choiceOfValues(logMediator, EsbPackage.eINSTANCE.getLogMediator_LogLevel()),
                        logMediator.getLogLevel());
            }
            if (isAccessible(EsbViewsRepository.LogMediator.Properties.logSeparator))
                basePart.setLogSeparator(
                        EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, logMediator.getLogSeparator()));

            if (isAccessible(EsbViewsRepository.LogMediator.Properties.properties_)) {
                propertiesSettings = new ReferencesTableSettings(logMediator,
            ...
            if (isAccessible(EsbViewsRepository.LogMediator.Properties.description))
                basePart.setDescription(
                        EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, logMediator.getDescription()));
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.LogMediator.Properties.logSeparator == event.getAffectedEditor()) {
            logMediator.setLogSeparator((java.lang.String) EEFConverterUtil
        ...
        if (EsbViewsRepository.LogMediator.Properties.description == event.getAffectedEditor()) {
            logMediator.setDescription((java.lang.String) EEFConverterUtil
                    .createFromString(EcorePackage.Literals.ESTRING, (String) event.getNewValue()));
        }
    }
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getLogMediator_LogCategory().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject)
                    && isAccessible(EsbViewsRepository.LogMediator.Properties.logCategory))
                basePart.setLogCategory((LogCategory) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getLogMediator_LogLevel().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject)
                    && isAccessible(EsbViewsRepository.LogMediator.Properties.logLevel))
                basePart.setLogLevel((LogLevel) msg.getNewValue());

            if (EsbPackage.eINSTANCE.getLogMediator_LogSeparator().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.LogMediator.Properties.logSeparator)) {
            ...
            if (EsbPackage.eINSTANCE.getLogMediator_Properties().equals(msg.getFeature())
                    && isAccessible(EsbViewsRepository.LogMediator.Properties.properties_))
                basePart.updateProperties();
            if (EsbPackage.eINSTANCE.getEsbElement_Description().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.LogMediator.Properties.description)) {
                if (msg.getNewValue() != null) {
                    basePart.setDescription(
                            EcoreUtil.convertToString(EcorePackage.Literals.ESTRING, msg.getNewValue()));
                } else {
    ...
    protected NotificationFilter[] getNotificationFilters() {
        NotificationFilter filter = new EStructuralFeatureNotificationFilter(
                EsbPackage.eINSTANCE.getLogMediator_LogCategory(), EsbPackage.eINSTANCE.getLogMediator_LogLevel(),
                EsbPackage.eINSTANCE.getLogMediator_LogSeparator(), EsbPackage.eINSTANCE.getLogMediator_Properties(),
                ...
                if (EsbViewsRepository.LogMediator.Properties.logCategory == event.getAffectedEditor()) {
                    ...
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getLogMediator_LogCategory().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getLogMediator_LogCategory().getEAttributeType(), newValue);
                }
                if (EsbViewsRepository.LogMediator.Properties.logLevel == event.getAffectedEditor()) {
                    Object newValue = event.getNewValue();
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getLogMediator_LogLevel().getEAttributeType(), (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getLogMediator_LogLevel().getEAttributeType(), newValue);
                }
                if (EsbViewsRepository.LogMediator.Properties.logSeparator == event.getAffectedEditor()) {
                    Object newValue = event.getNewValue();
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getLogMediator_LogSeparator().getEAttributeType(),
                                (String) newValue);
                    }
                    ret = Diagnostician.INSTANCE
                            .validate(EsbPackage.eINSTANCE.getLogMediator_LogSeparator().getEAttributeType(), newValue);
                }
                if (EsbViewsRepository.LogMediator.Properties.description == event.getAffectedEditor()) {
                    Object newValue = event.getNewValue();
                    if (newValue instanceof String) {
