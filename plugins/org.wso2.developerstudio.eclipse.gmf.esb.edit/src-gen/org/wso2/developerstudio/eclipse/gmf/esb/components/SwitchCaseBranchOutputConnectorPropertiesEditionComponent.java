...
public class SwitchCaseBranchOutputConnectorPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
     ...
     * 
     */
    public SwitchCaseBranchOutputConnectorPropertiesEditionComponent(PropertiesEditingContext editingContext,
            EObject switchCaseBranchOutputConnector, String editing_mode) {
    ...
    public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
        ...
        if (editingPart != null && key == partKey) {
            ...
            if (isAccessible(EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.commentMediators)) {
                commentMediatorsSettings = new ReferencesTableSettings(switchCaseBranchOutputConnector,
            ...
            if (isAccessible(EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.caseRegex))
                basePart.setCaseRegex(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING,
                        switchCaseBranchOutputConnector.getCaseRegex()));
    ...
    public void updateSemanticModel(final IPropertiesEditionEvent event) {
        ...
        if (EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.commentMediators == event
                .getAffectedEditor()) {
        ...
        if (EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.caseRegex == event.getAffectedEditor()) {
            switchCaseBranchOutputConnector.setCaseRegex((java.lang.String) EEFConverterUtil
    ...
    public void updatePart(Notification msg) {
        ...
        if (editingPart.isVisible()) {
            ...
            if (EsbPackage.eINSTANCE.getOutputConnector_CommentMediators().equals(msg.getFeature())
                    && isAccessible(EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.commentMediators))
                basePart.updateCommentMediators();
            if (EsbPackage.eINSTANCE.getSwitchCaseBranchOutputConnector_CaseRegex().equals(msg.getFeature())
                    && msg.getNotifier().equals(semanticObject) && basePart != null
                    && isAccessible(EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.caseRegex)) {
    ...
    public Diagnostic validateValue(IPropertiesEditionEvent event) {
        ...
        if (event.getNewValue() != null) {
            try {
                if (EsbViewsRepository.SwitchCaseBranchOutputConnector.Properties.caseRegex == event
                        .getAffectedEditor()) {
                    Object newValue = event.getNewValue();
                    if (newValue instanceof String) {
                        newValue = EEFConverterUtil.createFromString(
                                EsbPackage.eINSTANCE.getSwitchCaseBranchOutputConnector_CaseRegex().getEAttributeType(),
                                (String) newValue);
                    }
