...
public class CloneMediatorEditPart extends MultipleCompartmentComplexFiguredAbstractMediator {
	...
	public void notifyChanged(Notification notification) {
		...
		// this.getModel() will get EMF datamodel of the clone mediator datamodel
		if (this.getModel() instanceof CSSNodeImpl) {
			...
			if (model.getElement() instanceof CloneMediatorImpl) {
				...
				try {
					org.apache.synapse.mediators.eip.splitter.CloneMediator cloneMediator = CloneMediatorTransformer
							.createCloneMediator(new TransformationInfo(), (EsbNode) cloneMediatorDataModel);
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof CloneMediatorCloneIDEditPart) {
			((CloneMediatorCloneIDEditPart) childEditPart)
					.setLabel(getPrimaryShape().getFigureCloneMediatorPropertyValue());
			return true;
		}
	...
	public class CloneMediatorFigure extends CloneMediatorGraphicalShape {
		...
		private void createContents() {
			...
			fFigureCloneMediatorPropertyValue.setText(DEFAULT_PROPERTY_VALUE_TEXT);
		...
		public IFigure getToolTip() {
			if (StringUtils.isEmpty(toolTipMessage)) {
				toolTipMessage = Messages.CloneMediatorEditPart_ToolTipMessage;
			}
