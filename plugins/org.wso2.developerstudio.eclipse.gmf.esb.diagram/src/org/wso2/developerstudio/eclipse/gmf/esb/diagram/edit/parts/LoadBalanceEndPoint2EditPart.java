	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof LoadBalanceEndPointInputConnector2EditPart) {
			...
			if (((LoadBalanceEndPoint) parentEndpoint).getInputConnector().getIncomingLinks().size() != 0) {
				 ...
				 */
				position = ((source instanceof ComplexEndpoints)
						|| (source.eContainer().eContainer() instanceof Sequences)) ? 0.5 : 0.25;
			} else {
				position = ((this.getParent().getParent().getParent() instanceof ComplexEndpointsEditPart)
						|| (this.getParent().getParent().getParent() instanceof AbstractSequencesEditPart)) ? 0.5
								: 0.25;
			}
	...
	protected boolean removeFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof LoadBalanceEndPointInputConnector2EditPart) {
			getBorderedFigure().getBorderItemContainer()
					.remove(((LoadBalanceEndPointInputConnector2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof LoadBalanceEndPointOutputConnector2EditPart) {
			getBorderedFigure().getBorderItemContainer()
					.remove(((LoadBalanceEndPointOutputConnector2EditPart) childEditPart).getFigure());
			return true;
		}
		if (childEditPart instanceof LoadBalanceEndPointWestOutputConnector2EditPart) {
			getBorderedFigure().getBorderItemContainer()
					.remove(((LoadBalanceEndPointWestOutputConnector2EditPart) childEditPart).getFigure());
			return true;
		}
		return false;
	}
