...
public class ProxyServiceNameEditPart extends CompartmentEditPart implements ITextAwareEditPart {
	...
	public IParser getParser() {
		if (parser == null) {
			parser = EsbParserProvider.getParser(EsbElementTypes.ProxyService_3001, getParserElement(),
					EsbVisualIDRegistry.getType(
							org.wso2.developerstudio.eclipse.gmf.esb.diagram.edit.parts.ProxyServiceNameEditPart.VISUAL_ID));
		}
		return parser;
	}
	...
	protected void refreshFont() {
		FontStyle style = (FontStyle) getFontStyleOwnerView().getStyle(NotationPackage.eINSTANCE.getFontStyle());
		if (style != null) {
			FontData fontData = new FontData(style.getFontName(), 10,
					SWT.BOLD | (style.isItalic() ? SWT.ITALIC : SWT.NORMAL));
			setFont(fontData);
		}
	}
