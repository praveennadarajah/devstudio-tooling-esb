...
import static org.wso2.developerstudio.eclipse.gmf.esb.diagram.edit.parts.EditPartConstants.SPRING_MEDIATOR_ICON_PATH;

import org.apache.axiom.om.OMElement;
import org.apache.commons.lang.StringUtils;
import org.apache.synapse.SynapseException;
import org.apache.synapse.mediators.spring.SpringMediatorSerializer;
...
import org.wso2.developerstudio.eclipse.gmf.esb.impl.SpringMediatorImpl;
...
public class SpringMediatorEditPart extends FixedSizedAbstractMediator {
	...
	protected IFigure createNodeShape() {
		return primaryShape = new SpringMediatorFigure(new Color(null, 208, 67, 0)) {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		if (childEditPart instanceof SpringMediatorBeanNameEditPart) {
			((SpringMediatorBeanNameEditPart) childEditPart)
					.setLabel(getPrimaryShape().getFigureSpringMediatorPropertyValue());
			return true;
		}
		if (childEditPart instanceof SpringMediatorDescriptionEditPart) {
			((SpringMediatorDescriptionEditPart) childEditPart)
					.setLabel(getPrimaryShape().getSpringMediatorDescriptionLabel());
			return true;
		}
	...
	public class SpringMediatorFigure extends EsbGraphicalShapeWithLabel {
		 ...
		 * @generated
		 */
		public SpringMediatorFigure(Color borderColor) {
		...
		private void createContents() {
			...
			fFigureSpringMediatorPropertyValue.setText(DEFAULT_PROPERTY_VALUE_TEXT);
		...
		public String getIconPath() {
			return SPRING_MEDIATOR_ICON_PATH;
		}
		...
		public IFigure getToolTip() {
			if (StringUtils.isEmpty(toolTipMessage)) {
				toolTipMessage = Messages.SpringMediatorEditPart_ToolTipMessage;
			}
	...
	public void notifyChanged(Notification notification) {
		...
		if (this.getModel() instanceof CSSNodeImpl) {
			...
			if (model.getElement() instanceof SpringMediatorImpl) {
				SpringMediatorImpl springMediatorDataModel = (SpringMediatorImpl) model.getElement();
				try {
					org.apache.synapse.mediators.spring.SpringMediator springMediator = SpringMediatorTransformer
							.createSpringMediator((EsbNode) springMediatorDataModel);

					SpringMediatorSerializer springMediatorSerializer = new SpringMediatorSerializer();
					OMElement omElement = springMediatorSerializer.serializeSpecificMediator(springMediator);

					if (StringUtils
							.isEmpty(MediatorValidationUtil.validateMediatorsFromOEMElement(omElement, "spring"))) {
